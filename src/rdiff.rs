...
pub fn run_on_files(first_text: Vec<String>, second_text: Vec<String>) {
  let mut edit_trace = lcs_on_files(&first_text, &second_text);
  println!("shortest edit distance: {}", edit_trace.len());
  ses_on_files(&mut edit_trace, &first_text, &second_text);
}

// lcs produces the shortest edit sequence graph, given two strings
fn lcs_on_files(a: &Vec<String>, b: &Vec<String>) -> Vec<CircularArray> {
  // *** SETUP ***
  let n = a.len(); // len() returns num bytes, not num characters
  let m = b.len();
  println!("a is {} char long and b is {} char long", n, m);
  let max = n + m;
  println!("the max edits is {}", max);
...
fn ses_on_files(trace: &mut Vec<CircularArray>, a: &Vec<String>, b: &Vec<String>) {
  let mut diff: Vec<String> = vec![];
